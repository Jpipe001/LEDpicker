...
void setup() {
  ...
  Serial.printf("MDNS started ~\tTo Display, Use This Address : %s\n", SW_addr.c_str());
...
void Init_LEDS() {       // *********** Initialize LEDs
  ...
  //FastLED.addLeds<LED_TYPE, DATA_PIN, COLOR_ORDER>(leds, NUM_LEDS); // For WS2811
...
void Display_Black_LEDS() {       // *********** Reset to Black
...
void Display_Color_LEDS(int Red, int Green, int Blue) {       // *********** Set LEDS to Color
    fill_solid(leds, NUM_LEDS, CRGB::Black);  // Set All leds to Black
    FastLED.show();
...
String Web_Page() {         // Web Page Code
  ...
  //***********   BODY   ***********
  html_code += "<body><center><h1>ESP RGB Slider to HEX Converter</h1><P>";
  ...
  // Send the HEX value to the server using GET
  ...
  /*
    int search0 = html_code.indexOf("<TITLE>") + 7;
    int search1 = html_code.indexOf("</TITLE>");
    //Serial.printf("search0 = %d  search1 = %d\n", search0, search1);
    if (search0 > 1 && search1 > 1)   Serial.printf("html_code ~ TITLE = %s\n", html_code.substring(search0, search1).c_str());
    search0 = html_code.indexOf("const url = ") + 13;
    search1 = html_code.indexOf("fetch(url") - 25;
    //Serial.printf("search0 = %d  search1 = %d\n", search0, search1);
    if (search0 > 1 && search1 > 1)   Serial.printf("html_code ~   URL = %s\n", html_code.substring(search0, search1).c_str());
  */
  return html_code;
}
void loop() {
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            int search0 = header.indexOf("GET / HTTP/1.1");
            //Serial.printf("web page - search0 = %d\n", search0);
            ...
            search0 = header.indexOf("GET /?r=");
            if (search0 >= 0)  {
              search0 = header.indexOf("r=") + 2;
